    ...
    fn parse<'i>(
        ...
        mod rules {
            ...
            pub mod visible {
                    ...
                    state.rule(Rule::for_expr, |state| {
                        state.sequence(|state| {
                            state
                                ...
                                .and_then(|state| state.match_string("{"))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| self::stmts(state))
                                .and_then(|state| super::hidden::skip(state))
                                .and_then(|state| state.match_string("}"))
                        })
                    })
                ...
                {
                    state.rule(Rule::match_expr, |state| {
                        state.sequence(|state| {
                            state
                                .match_string("match")
                                .and_then(|state| super::hidden::skip(state))
                ...
                {
                    state.rule(Rule::object_expr, |state| {
                        state.sequence(|state| {
                            state
                                .match_string("{")
                                .and_then(|state| super::hidden::skip(state))
        ...
        ::pest::state(input, |state| match rule {
            ...
            Rule::match_expr => rules::match_expr(state),
            Rule::object_expr => rules::object_expr(state),
